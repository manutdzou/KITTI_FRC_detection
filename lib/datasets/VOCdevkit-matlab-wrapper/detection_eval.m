...
minoverlap=0.2;class_num=zeros(1,length(classes));
...
for i=1:length(classes)
    ...
    for j=1:nd
        ...
        if ovmax>=minoverlap
            tp(j)=1;
           imwrite(img(fix(bb_pred(2)):fix(bb_pred(4)),fix(bb_pred(1)):fix(bb_pred(3)),:),write_path1,'jpg');
        else
            fp(j)=1;
            imwrite(img(fix(bb_pred(2)):fix(bb_pred(4)),fix(bb_pred(1)):fix(bb_pred(3)),:),write_path2,'jpg');
        end
    end
    fp=cumsum(fp);
    tp=cumsum(tp);
